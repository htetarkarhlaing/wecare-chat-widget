...
import { setChatOpen, toggleChat as toggleChatAction } from '../store/chatSlice';
...
const normalizeLocale = (value?: string | null): ChatLocale | null => {
  ...
  return SUPPORTED_LOCALES.includes(base as ChatLocale) ? (base as ChatLocale) : null;
};
...
const detectLocale = (candidate?: string | null): ChatLocale => {
  ...
  if (typeof document !== 'undefined') {
    const attrLocale = normalizeLocale(document.documentElement.getAttribute('lang'));
...
export function ChatWidget({ config }: ChatWidgetProps) {
  ...
  React.useEffect(() => {
    if (resolvedConfig.theme?.primaryColor) {
      document.documentElement.style.setProperty('--chat-primary-color', resolvedConfig.theme.primaryColor);
    }
    if (resolvedConfig.theme?.secondaryColor) {
      document.documentElement.style.setProperty('--chat-secondary-color', resolvedConfig.theme.secondaryColor);
    }
  ...
  return (
    <div className={`chat-widget-container ${resolvedConfig.theme?.position || 'bottom-right'}`}>
      {!isFullscreen && (
        <ChatButton
          onClick={handleToggleChat}
          isOpen={isOpen}
          position={resolvedConfig.theme?.position || 'bottom-right'}
        />
      )}
      <ChatWindow isOpen={isOpen} onClose={closeChat} config={resolvedConfig} />
    </div>
  );
}